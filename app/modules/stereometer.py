...
class StereometerModule(BaseModule):
    ...
    def setup_settings(self):
        c = self.settings.add_combo("Display", ["Lissajous", "Linear", "Scaled"], 0)
        c.currentTextChanged.connect(lambda t: setattr(self, "_display_mode", t))
        c = self.settings.add_combo("Color", ["Static", "RGB", "Multi-Band"], 0)
        c.currentTextChanged.connect(lambda t: setattr(self, "_color_mode", t))
        c = self.settings.add_combo("Correlation", ["Single-Band", "Multi-Band"], 0)
        c.currentTextChanged.connect(lambda t: setattr(self, "_corr_mode", t))
    ...
    def _draw_dots(self, painter, left, right, cx, cy, radius, color, alpha):
        ...
        for i in range(0, n, step):
            ...
            if color is None:
                amp = abs(lv) + abs(rv)
                diff = abs(left[i] - left[max(0, i-1)]) + abs(right[i] - right[max(0, i-1)])
                rc = min(255, int(diff * 2000))
                gc = min(255, int(amp * 500))
                bc = max(0, 255 - rc - gc)
                dc = QColor(rc, gc, bc, alpha)
            else:
